...
class Blynk(BlynkProtocol):
    ...
    def connect(self):
        # Force disconnect to clear out any old broken socket states
        if self.state == CONNECTED: 
            return
            
        print('Connecting to %s:%d...' % (self.server, self.port))
        try:
            # Clean up old socket memory before starting a new one
            if self.conn:
                try: self.conn.close()
                except: pass
                self.conn = None
            ...
            self.state = DISCONNECTED # Reset state before protocol initialization
    ...
    def _write(self, data):
        ...
        try:
            ...
            print("[Errno 104] ECONNRESET")
            self.state = DISCONNECTED

    def run(self):
        if self.state == DISCONNECTED or not self.conn:
            # If we are disconnected, attempt reconnection
            self.connect()
            time.sleep(1)
            return
        ...
        try:
            ...
            raise
        except OSError: # Handles MicroPython timeouts safely
            pass
        except Exception as e: 
            print("Read connection lost:", e)
            self.state = DISCONNECTED
            return
